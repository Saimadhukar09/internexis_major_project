...
def fetch_information(title,poster,data,rating):
    global inc
    inc=inc+1 
    
    text[f'a{inc}'].config(text=title)
    if check_var.get():
        text2[f'a{inc}{inc}'].config(text=data)
    else:
...
def search():
    global inc
    inc=0
    request=Requests('GET',{'search':Search.get()})
    ...
    if response.status_code==200: 
        ...
        messagebox.showinfo("info","Failed to fetch from google books API")
...
recommend_button=Button(root,image=recommend_button_image,bg="#0099ff",bd=0,activebackground="#252532",cursor="hand1",command=search)
